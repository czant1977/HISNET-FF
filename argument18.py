...
def Local_rotation(ipath):
    ...
    bottom_left = cv2.flip(bottom_left, 0)  # 水平和垂直翻转
    bottom_right = cv2.flip(bottom_right, 1)  # 水平翻转

    # 将四个块放置回新图像的对应位置
    rotated_img[:half_height, :half_width] = top_left
    rotated_img[:half_height, half_width:] = top_right
    rotated_img[half_height:, :half_width] = bottom_left
    rotated_img[half_height:, half_width:] = bottom_right

    img_save_name = 'lr_' + os.path.split(ipath)[-1]
    img_save_path = os.path.join(save_path, img_save_name)
    
    # 保存图像
    cv2.imwrite(img_save_path, rotated_img)
    print("Operation: local_rotation img: {}".format(os.path.split(ipath)[-1]))
    return img_save_path
def self_augumentation(ipath,size=500):
...
if __name__ == '__main__':
    ...
    lei = 'head'
    class_num=1
    kind_path = os.path.join('./'+lei+'_5zhe_data/'+lei+'_data_'+str(class_num)+'/'+lei+'_51_r_p_5_aug/train')
    ...
    for d in kind_file:  # 一级目录Condylura等
        ...
        for i in imgs:  # i: Euroscaptor#grandis#57110#grandis_57110_DSC_3659#s#v.JPG
            ...
            img = cutout(ipath, n_holes=10, length=80,epoch=1)
